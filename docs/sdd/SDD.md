# TaijiFlow AI - Software Design Document (SDD)

**Version:** 1.1  
**Last Updated:** 2026-01-24  
**Status:** Final  
**Standard:** ISO/IEC 29110

---

## สารบัญ

1. [บทนำ (Introduction)](#1-บทนำ-introduction)
2. [การออกแบบสถาปัตยกรรม (Architectural Design)](#2-การออกแบบสถาปัตยกรรม-architectural-design)
3. [การออกแบบโมดูล (Module Design)](#3-การออกแบบโมดูล-module-design)
4. [การออกแบบข้อมูล (Data Design)](#4-การออกแบบข้อมูล-data-design)
5. [การออกแบบส่วนต่อประสาน (Interface Design)](#5-การออกแบบส่วนต่อประสาน-interface-design)
6. [การออกแบบ User Interface (UI Design)](#6-การออกแบบ-user-interface-ui-design)
7. [UML Diagrams](#7-uml-diagrams)
8. [Design Patterns](#8-design-patterns)
9. [Error Handling](#9-error-handling)
10. [Security Considerations](#10-security-considerations)
11. [Traceability Matrix (SRS → SDD)](#11-traceability-matrix-srs--sdd)
12. [ภาคผนวก (Appendix)](#12-ภาคผนวก-appendix)

---

## 1. บทนำ (Introduction)

### 1.1 วัตถุประสงค์ (Purpose)

เอกสารฉบับนี้อธิบายการออกแบบซอฟต์แวร์ของระบบ TaijiFlow AI ซึ่งเป็นระบบฝึกท่าม้วนไหม (Silk Reeling) ของมวยไท้เก๊กผ่านเว็บเบราว์เซอร์โดยใช้ AI ตรวจจับท่าทางและให้คำแนะนำแบบ Real-time

เอกสารนี้ระบุ:
- สถาปัตยกรรมระบบ (System Architecture)
- การออกแบบโมดูล (Module Design)
- การไหลของข้อมูล (Data Flow)
- ส่วนต่อประสาน (Interfaces)
- รูปแบบการออกแบบ (Design Patterns)

### 1.2 ขอบเขต (Scope)

| หัวข้อ | รายละเอียด |
|-------|-----------|
| **ชื่อระบบ** | TaijiFlow AI |
| **ประเภท** | Single Page Web Application |
| **สถาปัตยกรรม** | Client-Side Only (Static Hosting) |
| **เทคโนโลยีหลัก** | HTML5, CSS3, JavaScript (ES6+) |

### 1.3 เอกสารอ้างอิง (References)

| เอกสาร | รายละเอียด |
|--------|-----------|
| **SRS.md** | Software Requirements Specification v3.0 |
| **ARCHITECTURE.md** | System Architecture Documentation |
| **MODULE_DEPENDENCIES.md** | Module Dependencies Matrix |
| **UML Diagrams** | 15 ไฟล์ใน `/docs/diagrams/` |

### 1.4 คำศัพท์เฉพาะ (Definitions)

| คำศัพท์ | ความหมาย |
|--------|----------|
| **Silk Reeling** | ท่าม้วนไหม - ท่าพื้นฐานของมวยไท้เก๊กสกุลเฉิน |
| **Landmark** | จุดพิกัดของข้อต่อที่ตรวจจับโดย MediaPipe (33 จุด) |
| **Calibration** | การปรับเทียบสัดส่วนร่างกายด้วยท่า T-Pose |
| **Heuristics** | กฎการวิเคราะห์ท่าทางตามหลักไท้เก๊ก 9 ข้อ |
| **Ghost Overlay** | เงาครูผู้สอนที่แสดงซ้อนบนหน้าจอ |

---

## 2. การออกแบบสถาปัตยกรรม (Architectural Design)

### 2.1 ภาพรวมสถาปัตยกรรม (Architecture Overview)

TaijiFlow AI ใช้ **Client-Side Architecture** ประมวลผลทั้งหมดบนเบราว์เซอร์ ไม่มี Backend Server

```
┌─────────────────────────────────────────────────────────────────┐
│                     TaijiFlow AI Architecture                    │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  ┌──────────────┐    ┌──────────────┐    ┌──────────────┐       │
│  │   Browser    │    │   Webcam     │    │   Speaker    │       │
│  │   (Chrome)   │    │   (720p+)    │    │   (Audio)    │       │
│  └──────┬───────┘    └──────┬───────┘    └──────▲───────┘       │
│         │                   │                   │                │
│  ┌──────┴───────────────────┴───────────────────┴───────┐       │
│  │                    Web Application                    │       │
│  │  ┌─────────────────────────────────────────────────┐ │       │
│  │  │              Presentation Layer                 │ │       │
│  │  │   [UI Manager] [Drawing Manager] [Chatbot]      │ │       │
│  │  └─────────────────────────────────────────────────┘ │       │
│  │  ┌─────────────────────────────────────────────────┐ │       │
│  │  │             Business Logic Layer                │ │       │
│  │  │  [Heuristics] [Calibration] [Scoring] [Gesture] │ │       │
│  │  └─────────────────────────────────────────────────┘ │       │
│  │  ┌─────────────────────────────────────────────────┐ │       │
│  │  │                 Data Layer                      │ │       │
│  │  │  [Session] [Exporter] [Translations] [Storage]  │ │       │
│  │  └─────────────────────────────────────────────────┘ │       │
│  └───────────────────────┬───────────────────────────────┘       │
│                          │                                       │
│  ┌───────────────────────┴───────────────────────────────┐       │
│  │                   External APIs (CDN)                 │       │
│  │  [MediaPipe Pose] [MediaPipe Gesture] [Gemini API]   │       │
│  └───────────────────────────────────────────────────────┘       │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

### 2.2 Layer Architecture (4 Layers)

| Layer | หน้าที่ | Modules |
|-------|--------|---------|
| **Presentation** | แสดงผล UI, รับ input, วาด Canvas | ui_manager, drawing_manager, chatbot, tutorial_manager |
| **Business Logic** | ประมวลผลหลัก, วิเคราะห์ท่า | script.js, heuristics_engine, calibration_manager, scoring_manager |
| **Data** | จัดการข้อมูล, Session, Export | session_manager, data_exporter, translations |
| **External APIs** | บริการภายนอก | MediaPipe Pose, MediaPipe Gesture, Gemini API |

**หลักการ:** แต่ละ Layer สื่อสารกับ Layer ที่อยู่ติดกันเท่านั้น (Loose Coupling)

> 📐 **UML Diagram:** ดู [LayerArchitecture.wsd](../diagrams/LayerArchitecture.wsd)

### 2.3 Technology Stack

#### 2.3.1 Frontend Technologies

| Technology | Version | Purpose |
|------------|:-------:|---------|
| **HTML5** | - | Page Structure |
| **CSS3** | - | Styling |
| **JavaScript** | ES6+ | Core Logic (22 modules) |
| **TailwindCSS** | 3.x | Utility CSS Framework |
| **Google Fonts** | - | Sarabun Thai Font |

#### 2.3.2 AI / Machine Learning

| Technology | Version | Purpose |
|------------|:-------:|---------|
| **MediaPipe Pose** | @latest | Pose Detection (33 landmarks) |
| **MediaPipe Tasks Vision** | @0.10.8 | Hand Gesture Recognition (👍✊) |
| **Gemini API** | - | AI Chatbot (Optional) |

#### 2.3.3 Browser APIs

| API | Purpose |
|-----|---------|
| **Canvas API** | วาด Skeleton, Path, Trail, Ghost |
| **Web Speech API** | Text-to-Speech เสียงเตือน |
| **LocalStorage API** | บันทึก Calibration, Settings |
| **Fullscreen API** | โหมดเต็มจอ |
| **Fetch API** | ส่ง Bug Report |

### 2.4 File Structure

```
TaijiFlow/
├── 📄 index.html                    # Landing Page (Entry Point)
├── 📄 app.html                      # Training Application (Main App)
├── 📄 data_collector.html           # เครื่องมือเก็บข้อมูล Reference
│
├── 📁 css/                          # Stylesheets (5 files)
│   ├── base.css                     # Shared Styles (Variables, Reset, Components)
│   ├── styles.css                   # App Styles (app.html)
│   ├── landing.css                  # Landing Page Styles
│   ├── chatbot.css                  # Chatbot Popup Styles
│   └── feedback.css                 # Feedback Modal Styles
│
├── 📁 js/                           # JavaScript Modules (22 files)
│   ├── script.js                    # Main Controller (~72KB)
│   ├── heuristics_engine.js         # 9 Rules Analysis (~51KB)
│   ├── calibration_manager.js       # T-Pose Calibration (~15KB)
│   ├── scoring_manager.js           # Score Calculation (~11KB)
│   ├── drawing_manager.js           # Canvas Rendering (~25KB)
│   ├── ghost_manager.js             # Instructor Overlay (~8KB)
│   ├── silhouette_manager.js        # User Silhouette (~3KB)
│   ├── ui_manager.js                # Theme, Notifications (~41KB)
│   ├── audio_manager.js             # Text-to-Speech (~31KB)
│   ├── score_popup_manager.js       # Result Popup (~7KB)
│   ├── tutorial_manager.js          # Tutorial Popup (~30KB)
│   ├── gesture_manager.js           # Hand Control (~14KB)
│   ├── feedback_manager.js          # Bug Report (~4KB)
│   ├── chatbot.js                   # Gemini AI Chatbot (~26KB)
│   ├── rules_config_manager.js      # Settings UI (~12KB)
│   ├── display_controller.js        # Display Options (~7KB)
│   ├── keyboard_controller.js       # Keyboard Shortcuts (~9KB)
│   ├── data_exporter.js             # Export JSON/CSV (~8KB)
│   ├── path_generator.js            # Dynamic Path (~5KB)
│   ├── session_manager.js           # User/Session ID (~5KB)
│   ├── translations.js              # i18n TH/EN (~23KB)
│   └── silk-animation.js            # Landing Animation (~7KB)
│
├── 📁 data/                         # Reference Data
│   ├── {exercise}.json              # Ghost landmarks
│   ├── {exercise}.webm              # Instructor video
│   └── {exercise}_silhouette.webm   # Silhouette video
│
└── 📁 docs/                         # Documentation
    ├── diagrams/                    # UML Diagrams (15 files)
    ├── srs/SRS.md                   # Requirements
    └── sdd/SDD.md                   # This file
```

#### File Statistics

| Category | Files | Total Size |
|----------|:-----:|:----------:|
| Core Managers | 3 | ~77 KB |
| Display Managers | 3 | ~36 KB |
| UI Managers | 8 | ~165 KB |
| Controllers | 2 | ~16 KB |
| Utilities | 4 | ~41 KB |
| Main Controller | 1 | ~72 KB |
| Landing Page | 1 | ~7 KB |
| **Total JS** | **22** | **~414 KB** |

---

## 3. การออกแบบโมดูล (Module Design)

### 3.1 Module Overview

ระบบประกอบด้วย 22 JavaScript modules แบ่งเป็น 6 กลุ่ม:

```
┌─────────────────────────────────────────────────────────────────┐
│                        script.js                                 │
│                     (Main Controller)                            │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │                    Core Managers                          │   │
│  │  ┌───────────────┐ ┌───────────────┐ ┌───────────────┐   │   │
│  │  │ Heuristics    │ │ Calibration   │ │   Scoring     │   │   │
│  │  │  Engine       │ │   Manager     │ │   Manager     │   │   │
│  │  └───────────────┘ └───────────────┘ └───────────────┘   │   │
│  └──────────────────────────────────────────────────────────┘   │
│                                                                  │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │                   Display Managers                        │   │
│  │  ┌───────────────┐ ┌───────────────┐ ┌───────────────┐   │   │
│  │  │   Drawing     │ │    Ghost      │ │  Silhouette   │   │   │
│  │  │   Manager     │ │   Manager     │ │   Manager     │   │   │
│  │  └───────────────┘ └───────────────┘ └───────────────┘   │   │
│  └──────────────────────────────────────────────────────────┘   │
│                                                                  │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │                     UI Managers                           │   │
│  │  ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐        │   │
│  │  │   UI    │ │  Audio  │ │ Score   │ │Tutorial │        │   │
│  │  │ Manager │ │ Manager │ │ Popup   │ │ Manager │        │   │
│  │  └─────────┘ └─────────┘ └─────────┘ └─────────┘        │   │
│  │  ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐        │   │
│  │  │ Gesture │ │Feedback │ │ Chatbot │ │ Rules   │        │   │
│  │  │ Manager │ │ Manager │ │         │ │ Config  │        │   │
│  │  └─────────┘ └─────────┘ └─────────┘ └─────────┘        │   │
│  └──────────────────────────────────────────────────────────┘   │
│                                                                  │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │                      Controllers                          │   │
│  │  ┌───────────────────┐ ┌───────────────────┐             │   │
│  │  │ Display Controller│ │ Keyboard Controller│             │   │
│  │  └───────────────────┘ └───────────────────┘             │   │
│  └──────────────────────────────────────────────────────────┘   │
│                                                                  │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │                       Utilities                           │   │
│  │  ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐        │   │
│  │  │  Data   │ │  Path   │ │ Session │ │ Trans-  │        │   │
│  │  │Exporter │ │Generator│ │ Manager │ │ lations │        │   │
│  │  └─────────┘ └─────────┘ └─────────┘ └─────────┘        │   │
│  └──────────────────────────────────────────────────────────┘   │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

> 📐 **UML Diagram:** ดู [ClassDiagram.wsd](../diagrams/ClassDiagram.wsd)

### 3.2 Core Managers

#### 3.2.1 HeuristicsEngine

**หน้าที่:** วิเคราะห์ท่าทางตามหลักไท้เก๊ก 9 กฎ

```javascript
class HeuristicsEngine {
    // Configuration
    - CONFIG: object           // ค่าคงที่ (thresholds, weights)
    - calibrationData: object  // ข้อมูล T-Pose
    - RULES_CONFIG: object     // กฎแต่ละระดับ
    
    // Methods
    + analyze(landmarks): Feedback[]     // วิเคราะห์ท่าทาง
    + setCalibration(data)               // ตั้งค่า calibration
    + updateRuleConfig(level)            // อัปเดตกฎตามระดับ
    
    // 9 Rules (Private Methods)
    - checkPathShape()        // Rule 1: รูปทรงเส้นทาง
    - checkArmRotation()      // Rule 2: การหมุนแขน
    - checkElbowSinking()     // Rule 3: ศอกจม
    - checkWaistInitiation()  // Rule 4: เอวนำ
    - checkVerticalStability()// Rule 5: ความมั่นคง
    - checkSmoothness()       // Rule 6: ความนุ่มนวล
    - checkContinuity()       // Rule 7: ความต่อเนื่อง
    - checkWeightShift()      // Rule 8: ถ่ายน้ำหนัก
    - checkCoordination()     // Rule 9: บนล่างสอดคล้อง
}
```

**9 Heuristic Rules:**

| Rule ID | ชื่อกฎ | หลักการ | ระดับที่ใช้ |
|:-------:|--------|--------|:----------:|
| R-01 | Path Shape | เส้นทางมือเป็นวงรีไม่บิดเบี้ยว | L1, L2, L3 |
| R-02 | Arm Rotation | ปลายแขนหมุนตามทิศทางที่กำหนด | L2, L3 |
| R-03 | Elbow Sinking | ศอกต่ำกว่าไหล่ตลอดเวลา | L1, L2, L3 |
| R-04 | Waist Initiation | เอวนำการเคลื่อนไหวก่อนแขน | L3 |
| R-05 | Vertical Stability | ลำตัวตั้งตรงไม่เอียง | L1, L2, L3 |
| R-06 | Smoothness | ความเร็วสม่ำเสมอ ไม่กระตุก | L2, L3 |
| R-07 | Continuity | เคลื่อนไหวต่อเนื่องไม่หยุดชะงัก | L2, L3 |
| R-08 | Weight Shift | ถ่ายน้ำหนักซ้าย-ขวาตามจังหวะ | L3 |
| R-09 | Coordination | การประสานงานของร่างกายส่วนบนและล่าง | L3 |

#### 3.2.2 CalibrationManager

**หน้าที่:** ปรับเทียบสัดส่วนร่างกายด้วยท่า T-Pose

```javascript
class CalibrationManager {
    // State
    - isActive: boolean
    - level: string           // L1, L2, L3
    - calibrationData: object
    - frameBuffer: array      // 30 frames
    
    // Methods
    + start()                    // เริ่ม calibration
    + cancel()                   // ยกเลิก
    + process(landmarks): object // ประมวลผลทุก frame
    + getCalibrationData()       // ดึงผลลัพธ์
    
    // Validation
    - checkVisibility(landmarks) // ตรวจว่าเห็นครบ
    - checkTPose(landmarks)      // ตรวจท่า T-Pose
    - calculateDimensions()      // คำนวณสัดส่วน
}
```

**Calibration Data Structure:**

```javascript
calibrationData = {
    armLength: number,      // ความยาวแขน (pixels)
    shoulderWidth: number,  // ความกว้างไหล่
    torsoHeight: number,    // ความสูงลำตัว
    hipWidth: number,       // ความกว้างสะโพก
    level: string,          // L1, L2, L3
    timestamp: number       // เวลาที่ calibrate
}
```

#### 3.2.3 ScoringManager

**หน้าที่:** คำนวณคะแนนและสรุปผลการฝึก

```javascript
class ScoringManager {
    // State
    - isRecording: boolean
    - frameData: array         // ข้อมูลทุก frame
    - ruleScores: object       // คะแนนแต่ละกฎ
    
    // Methods
    + start()                        // เริ่มบันทึก
    + recordFrame(feedbacks)         // บันทึกผล frame
    + getSummary(): ScoreSummary     // สรุปผล
    - calculateGrade(score)          // ตัดเกรด A-F
    - getTopErrors(n)                // Top N ข้อผิดพลาด
}
```

**Score Summary Structure:**

```javascript
scoreSummary = {
    overallScore: number,    // 0-100%
    grade: string,           // A, B, C, D, F
    totalFrames: number,     // จำนวน frame ทั้งหมด
    durationSeconds: number, // เวลาฝึก
    ruleScores: {
        "R-01": number,      // คะแนนแต่ละกฎ
        "R-02": number,
        // ...
    },
    topErrors: [             // Top 3 ข้อผิดพลาด
        { rule: "R-03", count: 45, feedback: "ศอกสูงเกินไป" }
    ]
}
```

### 3.3 Display Managers

#### 3.3.1 DrawingManager

**หน้าที่:** วาดกราฟิกบน Canvas (Skeleton, Path, Trail)

```javascript
class DrawingManager {
    // Dependencies
    - ctx: CanvasRenderingContext2D
    - calibrationData: object
    
    // Methods
    + drawSkeleton(landmarks, color)    // วาดโครงกระดูก
    + drawPath(landmarks, exerciseType) // วาดเส้นทาง
    + drawTrail(trailHistory)           // วาดรอยทาง
    + drawDebugInfo(landmarks)          // แสดงข้อมูล debug
    + clear()                           // ล้าง canvas
}
```

**Drawing Order (Z-Index):**

| Order | Layer | Module | Blend Mode |
|:-----:|-------|--------|:----------:|
| 1 | Webcam Video | HTML Video | normal |
| 2 | Silhouette | silhouette_manager | lighten |
| 3 | Ghost Overlay | ghost_manager | lighten |
| 4 | Instructor Video | ghost_manager | normal |
| 5 | Path | drawing_manager | normal |
| 6 | Skeleton | drawing_manager | normal |
| 7 | Trail | drawing_manager | normal |

#### 3.3.2 GhostManager

**หน้าที่:** แสดงเงาครูผู้สอน (Video/Skeleton)

```javascript
class GhostManager {
    // State
    - isVisible: boolean
    - referenceData: object[]  // Ghost landmarks
    - videoElement: HTMLVideoElement
    
    // Methods
    + loadReference(exerciseId)      // โหลดข้อมูล
    + show() / hide()                // แสดง/ซ่อน
    + drawGhost(ctx, frameIndex)     // วาด skeleton
    + syncWithVideo(currentTime)     // sync กับวิดีโอ
}
```

### 3.4 Controllers

#### 3.4.1 DisplayController

**หน้าที่:** จัดการ Display Options 6 รูปแบบ

```javascript
class DisplayController {
    // Display Options
    - showGhostOverlay: boolean   // เงาครู
    - showPath: boolean           // เส้นทาง
    - showSkeleton: boolean       // โครงกระดูก
    - showSilhouette: boolean     // เงาผู้ฝึก
    - showTrail: boolean          // รอยทาง
    - trailHistory: array         // ประวัติจุด
    
    // Methods
    + init()                      // Initialize checkboxes
    + toggleInstructor(show)      // แสดง/ซ่อน instructor
    + resetToDefaults()           // รีเซ็ตค่า default
    + addTrailPoint(x, y)         // เพิ่มจุด trail
    + clearTrail()                // ล้าง trail
}
```

#### 3.4.2 KeyboardController

**หน้าที่:** จัดการ Keyboard Shortcuts 14 ปุ่ม

| Key | Action | Module |
|:---:|--------|--------|
| `Space` | Start/Stop Training | script.js |
| `R` | Reset Calibration | calibration_manager |
| `G` | Toggle Ghost | ghost_manager |
| `P` | Toggle Path | drawing_manager |
| `S` | Toggle Skeleton | drawing_manager |
| `T` | Toggle Trail | drawing_manager |
| `I` | Toggle Instructor | ghost_manager |
| `F` | Toggle Fullscreen | Browser API |
| `M` | Toggle Mute | audio_manager |
| `D` | Toggle Dark Mode | ui_manager |
| `L` | Toggle Language | translations |
| `H` | Toggle Debug | drawing_manager |
| `?` | Show Shortcuts | keyboard_controller |
| `Esc` | Cancel/Close | script.js |

---

## 4. การออกแบบข้อมูล (Data Design)

### 4.1 Data Flow Diagram

```
┌─────────┐    ┌──────────────┐    ┌─────────────────┐
│ Webcam  │───▶│ MediaPipe    │───▶│ 33 Landmarks    │
└─────────┘    │ Pose         │    │ (x, y, z, vis)  │
               └──────────────┘    └────────┬────────┘
                                            │
                    ┌───────────────────────┴───────────────────┐
                    ▼                                           ▼
            ┌───────────────┐                           ┌───────────────┐
            │ Calibration   │                           │  Heuristics   │
            │ Manager       │                           │  Engine       │
            └───────┬───────┘                           └───────┬───────┘
                    │                                           │
                    ▼                                           ▼
            ┌───────────────┐                           ┌───────────────┐
            │ calibration   │                           │ feedbacks[]   │
            │ Data          │                           │               │
            └───────────────┘                           └───────┬───────┘
                                                                │
                                    ┌───────────────────────────┴───────┐
                                    ▼                                   ▼
                            ┌───────────────┐                   ┌───────────────┐
                            │ Scoring       │                   │ UI Manager    │
                            │ Manager       │                   │ (Display)     │
                            └───────────────┘                   └───────────────┘
```

### 4.2 Data Structures

#### 4.2.1 Pose Landmark

```javascript
landmark = {
    x: number,          // 0.0 - 1.0 (normalized)
    y: number,          // 0.0 - 1.0 (normalized)
    z: number,          // ความลึก (ค่าลบ = ใกล้กล้อง)
    visibility: number  // 0.0 - 1.0 (confidence)
}
```

**33 Landmark Points:**

| Index | Name | Index | Name |
|:-----:|------|:-----:|------|
| 0 | nose | 17 | right_pinky |
| 11 | left_shoulder | 18 | left_index |
| 12 | right_shoulder | 19 | right_index |
| 13 | left_elbow | 20 | left_thumb |
| 14 | right_elbow | 21 | right_thumb |
| 15 | left_wrist | 23 | left_hip |
| 16 | right_wrist | 24 | right_hip |

#### 4.2.2 Feedback Object

```javascript
feedback = {
    rule: string,       // "R-01" - "R-09"
    type: string,       // "error", "warning", "info"
    message: string,    // ข้อความเตือน
    landmark: string,   // จุดที่ผิด (optional)
    severity: number    // 1-3 (ความรุนแรง)
}
```

#### 4.2.3 Reference Data (JSON)

```javascript
// data/rh_cw_L1.json
referenceData = {
    exerciseId: "rh_cw_L1",
    exerciseName: "Right Hand Clockwise Level 1",
    frames: [
        {
            frameIndex: 0,
            landmarks: [ /* 33 landmarks */ ],
            timestamp: 0.0
        },
        // ... ~600 frames (20 seconds @ 30fps)
    ],
    metadata: {
        duration: 20.0,
        fps: 30,
        totalFrames: 600
    }
}
```

### 4.3 Storage Design

#### 4.3.1 LocalStorage Keys

| Key | Data Type | Purpose |
|-----|-----------|---------|
| `taijiflow_calibration` | JSON | ข้อมูล calibration |
| `taijiflow_settings` | JSON | ค่าตั้งค่า (theme, language) |
| `taijiflow_session` | JSON | Session ID, User ID |
| `taijiflow_rules_config` | JSON | ค่ากฎที่ปรับแต่ง |

#### 4.3.2 Settings Structure

```javascript
settings = {
    theme: "dark" | "light",
    language: "th" | "en",
    audioEnabled: boolean,
    audioVolume: number,     // 0.0 - 1.0
    displayOptions: {
        showGhost: boolean,
        showPath: boolean,
        showSkeleton: boolean,
        showSilhouette: boolean,
        showTrail: boolean
    }
}
```

---

## 5. การออกแบบส่วนต่อประสาน (Interface Design)

### 5.1 Module Interfaces

#### 5.1.1 script.js (Main Controller)

```javascript
// Entry Point - เรียกจาก app.html
async function initializeApp() {
    await loadMediaPipe();
    initializeManagers();
    setupEventListeners();
    startCamera();
}

// Training Flow
function startTrainingFlow() { }
function endTrainingSession() { }

// MediaPipe Callback
function onResults(results) {
    // เรียกทุก frame (~30 fps)
    // results.poseLandmarks = 33 points
}
```

#### 5.1.2 Inter-Module Communication

```javascript
// Pattern: Direct Method Calls
calibrationManager.start();
const feedbacks = heuristicsEngine.analyze(landmarks);
scoringManager.recordFrame(feedbacks);

// Pattern: Event-Based (ใช้กับ UI)
document.dispatchEvent(new CustomEvent('languageChange', {
    detail: { language: 'en' }
}));

// Pattern: Callback
audioManager.speak(message, {
    onEnd: () => { /* speaking finished */ }
});
```

### 5.2 External API Interfaces

#### 5.2.1 MediaPipe Pose Landmarker

```javascript
const poseLandmarker = await PoseLandmarker.createFromOptions(vision, {
    baseOptions: {
        modelAssetPath: "pose_landmarker_full.task",
        delegate: "GPU"
    },
    runningMode: "VIDEO",
    numPoses: 1,
    minPoseDetectionConfidence: 0.5,
    minPosePresenceConfidence: 0.5,
    minTrackingConfidence: 0.5
});

// Detect
const results = poseLandmarker.detectForVideo(video, timestamp);
// results.landmarks[0] = 33 NormalizedLandmark
```

#### 5.2.2 MediaPipe Gesture Recognizer

```javascript
const gestureRecognizer = await GestureRecognizer.createFromOptions(vision, {
    baseOptions: {
        modelAssetPath: "gesture_recognizer.task",
        delegate: "GPU"
    },
    runningMode: "VIDEO",
    numHands: 2
});

// Recognize
const results = gestureRecognizer.recognizeForVideo(video, timestamp);
// results.gestures[0][0].categoryName = "Thumb_Up" | "Closed_Fist"
```

#### 5.2.3 Gemini API (Optional)

```javascript
const response = await fetch(`https://generativelanguage.googleapis.com/v1/models/gemini-pro:generateContent?key=${API_KEY}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
        contents: [{
            parts: [{ text: userMessage }]
        }],
        systemInstruction: {
            parts: [{ text: SYSTEM_PROMPT }]
        }
    })
});
```

---

## 6. การออกแบบ User Interface (UI Design)

### 6.1 Screen Layout

#### 6.1.1 Landing Page (index.html)

```
┌─────────────────────────────────────────────────────────────────┐
│ ☯️ TaijiFlow AI    หน้าแรก  เกี่ยวกับ  คู่มือ  อ้างอิง  [เริ่มฝึก]│
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│                     [Silk Animation]                             │
│                                                                  │
│                     TaijiFlow AI                                 │
│                 ระบบ AI เพื่อช่วยฝึก Silk Reeling                │
│                                                                  │
│                 [เริ่มการฝึก]  [เกี่ยวกับโครงการ]                │
│                                                                  │
├─────────────────────────────────────────────────────────────────┤
│ 🎯 เกี่ยวกับโครงการ                                              │
│ ┌───────────┐ ┌───────────┐ ┌───────────┐ ┌───────────┐        │
│ │ ที่มา     │ │ เทคโนโลยี  │ │ จุดเด่น   │ │ กิตติกรรม │        │
│ └───────────┘ └───────────┘ └───────────┘ └───────────┘        │
├─────────────────────────────────────────────────────────────────┤
│ 📚 คู่มือการใช้งาน                                               │
│     [1] เลือกท่า  →  [2] เลือกระดับ  →  [3] กดเริ่มฝึก          │
├─────────────────────────────────────────────────────────────────┤
│ 📚 เรียนรู้เพิ่มเติม                                             │
│ ┌───────────┐ ┌───────────┐ ┌───────────┐                      │
│ │ วิดีโอ    │ │ หลักการ   │ │ 8 กฎ     │                      │
│ └───────────┘ └───────────┘ └───────────┘                      │
├─────────────────────────────────────────────────────────────────┤
│                     © 2026 TaijiFlow AI                         │
└─────────────────────────────────────────────────────────────────┘
```

#### 6.1.2 Training Application (app.html)

```
┌─────────────────────────────────────────────────────────────────┐
│ ☯️ TaijiFlow AI    [Exercise▼] [Level▼]  ⚙️ 🌙 ❓  [▶ เริ่มฝึก] │
├─────────────────────────────────────────────────────────────────┤
│ ┌─────────────────────────────────────────────────────────────┐ │
│ │                                              ┌─────────┐    │ │
│ │       [Webcam + Canvas Overlay]              │Instructor│   │ │
│ │                                              │ Video   │    │ │
│ │    ┌─────────────────────────────┐          └─────────┘    │ │
│ │    │  [Start Overlay]            │                         │ │
│ │    │  ☯️ พร้อมเริ่มฝึก           │                         │ │
│ │    │  -----                      │                         │ │
│ │    │  👍 ยกขึ้น หรือ กด Space    │                         │ │
│ │    │  ✊ กำมือ หรือ กด Esc เพื่อหยุด│                        │ │
│ │    └─────────────────────────────┘                         │ │
│ │                                                            │ │
│ │  [Silhouette] [Ghost] [Path] [Skeleton] [Trail]            │ │
│ │                                                            │ │
│ │  ┌───────────────────────────────────────────────────────┐ │ │
│ │  │ 🔊 ศอกสูงเกินไป - ลองหย่อนศอกลงอีกนิด              │ │ │
│ │  └───────────────────────────────────────────────────────┘ │ │
│ └─────────────────────────────────────────────────────────────┘ │
├─────────────────────────────────────────────────────────────────┤
│ Display: ☑️Ghost ☑️Path ☑️Skeleton ☐Silhouette ☐Trail         │
│ Keyboard Shortcuts: Space(Start) G(Ghost) P(Path) S(Skel) ?    │
└─────────────────────────────────────────────────────────────────┘
```

### 6.2 UI Components

#### 6.2.1 Score Popup

```
┌─────────────────────────────────────────────┐
│              🏆 สรุปผลการฝึก                 │
├─────────────────────────────────────────────┤
│                                             │
│               ⭐⭐⭐⭐⭐                     │
│               เกรด: A                        │
│                                             │
│      คะแนนรวม: 92.5%  เวลา: 02:30           │
│                                             │
│  ┌─────────────────────────────────────┐   │
│  │ ⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜ Path Shape 95%  │   │
│  │ ⬜⬜⬜⬜⬜⬜⬜⬜⬜⬛ Elbow Sink 90%  │   │
│  │ ⬜⬜⬜⬜⬜⬜⬜⬜⬛⬛ Stability 88%   │   │
│  └─────────────────────────────────────┘   │
│                                             │
│  📉 จุดที่ควรปรับปรุง:                       │
│  • ศอกสูงเกินไป (12 ครั้ง)                  │
│  • เคลื่อนไหวไม่ต่อเนื่อง (8 ครั้ง)          │
│                                             │
│  [🔄 ฝึกอีกครั้ง]  [📊 ดูสถิติ]  [✕ ปิด]    │
│                                             │
└─────────────────────────────────────────────┘
```

---

## 7. UML Diagrams

### 7.1 Diagram Index

| Diagram Type | File | Description |
|--------------|------|-------------|
| **Use Case** | UseCaseDiagram.wsd | 6 Use Cases (UC-01 to UC-06) |
| **Class** | ClassDiagram.wsd | 22 Classes ใน 6 Packages |
| **Sequence** | SequenceDiagram_TrainingFlow.wsd | Training Flow หลัก |
| **Sequence** | SequenceDiagram_RealtimeAnalysis.wsd | Real-time Pose Analysis |
| **Sequence** | SequenceDiagram_Calibration.wsd | Calibration Process |
| **State** | StateDiagram_TrainingSession.wsd | 5 States ของ Training Session |
| **Activity** | ActivityDiagram_UC01.wsd | UC-01: System Settings |
| **Activity** | ActivityDiagram_UC02.wsd | UC-02: Perform Training |
| **Activity** | ActivityDiagram_UC03.wsd | UC-03: View Results |
| **Activity** | ActivityDiagram_UC04.wsd | UC-04: Tutorial |
| **Activity** | ActivityDiagram_UC05.wsd | UC-05: AI Chatbot |
| **Activity** | ActivityDiagram_UC06.wsd | UC-06: Manage Reference Data |
| **Activity** | ActivityDiagram_Heuristics.wsd | 9 Heuristic Rules Flow |
| **Component** | SystemArchitecture.wsd | ภาพรวมสถาปัตยกรรมระบบ |
| **Component** | ModuleDependencies.wsd | 22 Modules Dependencies |
| **Component** | LayerArchitecture.wsd | 4-Layer Architecture |
| **Data Flow** | DataFlowDiagram.wsd | การไหลของข้อมูลในระบบ |

### 7.2 Key Diagrams

> 📐 **ดู UML Diagrams ทั้งหมดที่:** [/docs/diagrams/](../diagrams/)

#### 7.2.1 Use Case Diagram Overview

```text
                        ┌──────────────────────────────────────────────────┐
                        │             TaijiFlow AI System                   │
                        ├──────────────────────────────────────────────────┤
                        │                                                   │
                        │  ┌─────────────────────────────────────────────┐ │
                        │  │         Training System (ผู้ฝึก)             │ │
                        │  │  (UC-01) Calibrate Body                     │ │
   ┌──────────┐        │  │  (UC-02) Perform Training  ◄─────────┐      │ │
   │  Trainee │────────│  │  (UC-03) View Training Results        │      │ │
   │  (ผู้ฝึก) │        │  │              ▲                       │      │ │
   └──────────┘        │  │              │<<include>>             │      │ │
                        │  │              └───UC-02────────────────┘      │ │
                        │  └─────────────────────────────────────────────┘ │
                        │                                                   │
                        │  ┌─────────────────────────────────────────────┐ │
                        │  │         User Support (สนับสนุน)             │ │
   ┌──────────┐        │  │  (UC-04) View Tutorial                      │ │
   │  Trainee │────────│  │  (UC-05) Manage Settings                    │ │
   └──────────┘        │  └─────────────────────────────────────────────┘ │
                        │                                                   │
                        │  ┌─────────────────────────────────────────────┐ │
                        │  │         Admin Tools (ผู้ดูแล)               │ │
   ┌──────────┐        │  │  (UC-06) Manage Reference Data              │ │
   │  Admin/  │────────│  │         (จัดการท่าต้นแบบ)                   │ │
   │Instructor│        │  └─────────────────────────────────────────────┘ │
   └──────────┘        │                                                   │
                        └──────────────────────────────────────────────────┘
```

> 📐 **UML Diagram:** ดู [UseCaseDiagram.wsd](../diagrams/UseCaseDiagram.wsd)

#### 7.2.2 State Diagram - Training Session

```
┌─────────────────────────────────────────────────────────────────┐
│                   Training Session States                        │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  [*] ──▶ ○ IDLE                                                 │
│              │                                                   │
│              │ start()                                           │
│              ▼                                                   │
│          ○ CALIBRATING                                          │
│              │                                                   │
│              │ calibration complete                              │
│              ▼                                                   │
│          ○ COUNTDOWN (3-2-1)                                    │
│              │                                                   │
│              │ countdown = 0                                     │
│              ▼                                                   │
│          ○ TRAINING  ◄───────────────┐                          │
│              │                        │                          │
│              │ stop()                 │ restart()                │
│              ▼                        │                          │
│          ○ ENDED ────────────────────┘                          │
│              │                                                   │
│              │                                                   │
│              ▼                                                   │
│            [*]                                                   │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

---

## 8. Design Patterns

### 8.1 Patterns Used

| Pattern | Where Used | Purpose |
|---------|------------|---------|
| **Module Pattern** | ทุก Manager classes | Encapsulation - แยก scope |
| **Singleton** | ghostManager, uiManager | Single Instance - ป้องกันสร้างซ้ำ |
| **Observer** | Event Listeners | Reactive UI - อัปเดตเมื่อ state เปลี่ยน |
| **Facade** | script.js | Simplify Access - รวม dependencies |
| **Factory** | generateDynamicPath() | Create Objects - สร้าง path ตามสัดส่วน |
| **Strategy** | HeuristicsEngine rules | Swappable Algorithms - เปิด/ปิดกฎได้ |
| **Controller** | KeyboardController | Separation of Concerns |

### 8.2 Pattern Examples

```javascript
// Singleton Pattern
const ghostManager = new GhostManager();
window.ghostManager = ghostManager;

// Module Pattern
class HeuristicsEngine {
    constructor() {
        this.CONFIG = { /* Private config */ };
    }
    analyze() { /* Public method */ }
}

// Observer Pattern
document.addEventListener('languageChange', () => {
    uiManager.updateAllTexts();
});

// Strategy Pattern
this.RULES_CONFIG = {
    L1: { checkPath: true, checkWaist: false },
    L3: { checkPath: true, checkWaist: true }
};

// Factory Pattern
function generateDynamicPath(calibration, exercise) {
    const { armLength, shoulderWidth } = calibration;
    return createEllipsePath(armLength * 0.8, shoulderWidth * 0.6);
}
```

---

## 9. Error Handling

### 9.1 Error Categories

| Category | Example | Handling |
|----------|---------|----------|
| **Camera Error** | ไม่พบ Webcam | แสดง modal แจ้งเตือน + คำแนะนำ |
| **MediaPipe Error** | โหลด model ไม่สำเร็จ | Retry 3 ครั้ง + fallback message |
| **Network Error** | ส่ง Bug Report ไม่สำเร็จ | แสดง notification + เก็บไว้ใน localStorage |
| **Calibration Error** | T-Pose ไม่ถูกต้อง | แสดงคำแนะนำ + ให้ลองใหม่ |
| **Runtime Error** | JavaScript exception | Log to console + notification |

### 9.2 Error Handling Flow

```javascript
// Global Error Handler
window.onerror = (message, source, lineno, colno, error) => {
    console.error('Global error:', error);
    uiManager.showNotification('เกิดข้อผิดพลาด กรุณาลองใหม่', 'error');
};

// Camera Error
async function startCamera() {
    try {
        const stream = await navigator.mediaDevices.getUserMedia({
            video: { width: 1280, height: 720 }
        });
    } catch (error) {
        if (error.name === 'NotAllowedError') {
            showCameraPermissionModal();
        } else if (error.name === 'NotFoundError') {
            showNoCameraModal();
        }
    }
}

// MediaPipe Error
async function loadMediaPipe() {
    for (let attempt = 1; attempt <= 3; attempt++) {
        try {
            await poseLandmarker.createFromOptions(vision, options);
            return;
        } catch (error) {
            if (attempt === 3) {
                uiManager.showNotification('โหลด AI Model ไม่สำเร็จ', 'error');
            }
            await delay(1000 * attempt);
        }
    }
}
```

---

## 10. Security Considerations

### 10.1 Data Privacy

| Data Type | Storage | Privacy |
|-----------|---------|---------|
| **Webcam Video** | Memory only | ไม่บันทึก ไม่ส่งออก |
| **Pose Landmarks** | Memory only | ประมวลผลในเครื่องเท่านั้น |
| **Calibration Data** | LocalStorage | เก็บในเครื่องผู้ใช้ |
| **Settings** | LocalStorage | เก็บในเครื่องผู้ใช้ |
| **Bug Reports** | Google Form | ส่งเฉพาะเมื่อผู้ใช้กดส่ง |

### 10.2 Security Measures

1. **No Server-Side Processing**: ข้อมูลทั้งหมดประมวลผลบน Client
2. **No Personal Data Collection**: ไม่เก็บข้อมูลส่วนบุคคล
3. **HTTPS Only**: บังคับใช้ HTTPS สำหรับ production
4. **Content Security Policy**: ป้องกัน XSS attacks
5. **API Key Protection**: Gemini API Key เก็บใน environment variable

### 10.3 API Key Handling

```javascript
// ❌ ไม่ควรทำ - API Key ใน code
const API_KEY = "AIza...";

// ✅ แนะนำ - ใช้ environment variable หรือ proxy
const API_KEY = import.meta.env.VITE_GEMINI_API_KEY;

// หรือใช้ serverless function เป็น proxy
const response = await fetch('/api/chat', {
    method: 'POST',
    body: JSON.stringify({ message })
});
```

---

## 11. Traceability Matrix (SRS → SDD)

### 11.1 Functional Requirements to Modules

| F-ID | Requirement | Primary Module | Supporting Modules |
|:----:|-------------|----------------|-------------------|
| F-01 | Body Calibration | calibration_manager.js | script.js, ui_manager.js |
| F-02 | Silk Reeling Training | script.js | heuristics_engine.js, drawing_manager.js |
| F-03 | 9 Rules Analysis | heuristics_engine.js | scoring_manager.js |
| F-04 | Real-time Feedback | audio_manager.js, ui_manager.js | heuristics_engine.js |
| F-05 | Training Result | score_popup_manager.js, scoring_manager.js | - |
| F-06 | Tutorial | tutorial_manager.js | translations.js |
| F-07 | System Settings | ui_manager.js, rules_config_manager.js | LocalStorage |
| F-08 | Display Options | display_controller.js | drawing_manager.js, ghost_manager.js |
| F-09 | Gesture Control | gesture_manager.js | script.js |
| F-10 | AI Chatbot | chatbot.js | Gemini API |
| F-11 | Feedback Report | feedback_manager.js | Google Form |
| F-12 | Reference Data Management | data_collector.html | data_exporter.js |

### 11.2 Non-Functional Requirements to Design

| NFR-ID | Requirement | Design Solution |
|:------:|-------------|-----------------|
| NFR-01 | Response Time < 100ms | Client-side processing, GPU delegate |
| NFR-02 | 30 FPS Minimum | requestAnimationFrame, skip frames if needed |
| NFR-03 | Works Offline (after load) | All resources cached, no server dependency |
| NFR-04 | Thai/English Support | translations.js with 200+ keys |
| NFR-05 | Dark/Light Theme | CSS variables, localStorage persistence |
| NFR-06 | Privacy | Client-only processing, no data upload |

---

## 12. ภาคผนวก (Appendix)

### 12.1 Glossary

| Term | Definition |
|------|------------|
| **Canvas** | HTML5 element สำหรับวาดกราฟิก 2D |
| **CDN** | Content Delivery Network - เซิร์ฟเวอร์กระจายเนื้อหา |
| **Landmark** | จุดพิกัดของข้อต่อที่ตรวจจับโดย AI |
| **MediaPipe** | AI Framework ของ Google สำหรับ Vision |
| **Pose** | ท่าทางของร่างกายที่ตรวจจับได้ |
| **Skeleton** | โครงกระดูกที่วาดจาก landmarks |
| **TTS** | Text-to-Speech - แปลงข้อความเป็นเสียง |

### 12.2 Version History

| Version | Date | Author | Changes |
|:-------:|------|--------|---------|
| 1.0 | 2026-01-09 | TaijiFlow Team | Initial SDD based on ISO 29110 |
| 1.1 | 2026-01-24 | TaijiFlow Team | Updated to 9 Rules (Coordination) |

### 12.3 Document Approval

| Role | Name | Date | Signature |
|------|------|------|-----------|
| Author | - | 2026-01-09 | - |
| Reviewer | - | - | - |
| Approver | - | - | - |

---

**End of Document**
