...
This document summarizes the comprehensive updates made to **Chapter 4: System Design** (`docs/thesis/chapter4.md`) to ensure completeness, accuracy, and visual alignment with the current codebase.
...
- **Coverage:** No file was omitted. `script.js` (Main Controller) and support utilities (`session_manager.js`, `data_exporter.js`) are now fully documented.
...
The documentation now accurately reflects the codebase as of **Version 0.9.1**. All 22 modules correspond 1:1 with the files in `js/`, and visual elements match the actual application UI.
...
- **CalibrationManager:** Added the `stop()` method to the class definition.
- **Sequence Diagrams:** Updated `SequenceDiagram_TrainingFlow.wsd` and `SequenceDiagram_Calibration.wsd` to include the `stop()` call after calibration completion, reflecting the fix for the infinite loop.
- **Class Diagram:** Updated `ClassDiagram.wsd` to include `stop()` in `CalibrationManager`.
...
- **Change:** Changed audio from "หยุดการฝึก" (Stop training) to "สิ้นสุดการฝึก" (Training ended).
...
- **Refactor:** Moved all hardcoded strings (Camera Errors, Announcements, Low Light Warnings) from `script.js` and `audio_manager.js` to `translations.js`.
...
- **UI:** Added "🎲 Random (Surprise Me)" option to the Exercise Dropdown.
- **Logic:**
    - When selected, `currentExercise` is set to "random".
...
### 7.2 UI Flow Reordering (Reverted)
- **Status:** Reverted to Original Flow (`Exercise` -> `Level`).
- **Reason:** To align with future "Double Hand" category hierarchy and user mental model (Select Content -> Select Difficulty).
- **Random Option:** Placed at the *bottom* of the Exercise dropdown (`[Index 5]`) to distinguish it from standard curriculum.

### 7.3 Debugging: "Double Click to Start" Fix
- **Issue:** Users had to click "Start" twice when using Random Mode.
- **Root Cause 1:** Race Condition between `loadReferenceData()` (async) and `calibrator.start()`.
- **Root Cause 2:** Crash due to missing function `uiManager.getExerciseName()` (did not exist).
- **Fix:**
    1. Replaced missing function with `uiManager.getText()`.
    2. Added `await` to `loadReferenceData()` to ensure data is ready before calibration overlay appears.

## 8. Documentation Updates (Thesis Chapter 5)

We updated `docs/thesis/chapter5.md` to reflect the latest feature set:

- **Random Exercise Mode (5.3.8):** Added section explaining the randomization logic and the race condition fix.
- **Environmental Checks (5.3.6):** Updated to correctly describe "Visibility-based Detection" + "Startup Delay" (removing incorrect Pixel-based references).
- **UI Layout (5.4.2):** Updated to mention the "Random Mode" menu and current header structure.
- **Diagrams:** Updated `SequenceDiagram_TrainingFlow.wsd` to include the Random Mode logic block.

## 9. Score Summary UI Redesign (v2.2)
Final polish optimizing for clarity and ease of access.

**Key Changes:**
1.  **Always-On Feedback**: QR Code is now visible by default with a clear invitation ("Help improve TaijiFlow AI").
2.  **Typography**: Increased font sizes for key stats (Correct/Error/Time) to improve readability at a distance.
3.  **Lightweight Overlay**: Reduced background opacity and removed blur to make the popup feel lighter and less intrusive.
4.  **Visual Consistency**:
    -   Close Button locked to **Theme Purple**.
    -   Stats displayed in a clean 3-column grid.
