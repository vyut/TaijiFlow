# ภาคผนวก ข: เอกสาร Software Requirements Specification (SRS)
...
| รายการ | รายละเอียด |
|--------|-----------| 
| **ชื่อเอกสาร** | TaijiFlow AI - Software Requirements Specification |
| **Version** | 3.0 |
| **สถานะ** | Final |
...
## ข.3 บทนำ (Introduction)

### ข.3.1 วัตถุประสงค์ (Purpose)
...
## ข.4 รายละเอียดผลิตภัณฑ์ (Product Description)

### ข.4.1 ฟีเจอร์ของระบบ (Product Functions)

#### ฟีเจอร์สำหรับผู้ฝึกฝน (Trainee) - 11 ฟีเจอร์
...
| F-08 | **Display Options** | ปรับการแสดงผล 7 รูปแบบ |
| F-09 | **Gesture Control** | ควบคุมด้วยท่าทางมือ (👍 เริ่ม, ✊ หยุด) |
| F-10 | **AI Chatbot** | ถาม-ตอบเกี่ยวกับไท่จี๋ (Gemini API) |
| F-11 | **Feedback Report** | ส่ง feedback/bug report |

#### ฟีเจอร์สำหรับผู้ดูแลระบบ (Admin) - 1 ฟีเจอร์

| F-ID | ฟีเจอร์ | คำอธิบาย |
|:----:|---------|---------| 
| F-12 | **Reference Data Management** | บันทึกวิดีโอและพิกัดท่าต้นแบบ |

### ข.4.2 กฎการตรวจสอบท่าทาง 9 ข้อ (9 Taijiquan Rules)
...
| R-09 | Upper-Lower Coordination | บนล่างสัมพันธ์ | 上下相随 |

---

## ข.5 Use Cases

### ข.5.1 Use Case Diagram
...
### ข.5.3 Data Flow Diagram

> 📐 **UML Diagram:** ดู [DataFlowDiagram.wsd](../../diagrams/DataFlowDiagram.wsd)
...
|--------|-------------|-----------| 
| 👤 User | ผู้ใช้งานระบบ | Webcam input, Score output |
| 🤖 MediaPipe | AI Model | 33 Landmarks |
| 📁 Reference Data | ไฟล์ JSON/WebM | Ghost Landmarks |
| 💾 LocalStorage | Browser Storage | Settings, Calibration |

| Process | Name | Input → Output |
|:-------:|------|----------------|
| 1.0 | Capture Video | Webcam → Video Frame |
| 2.0 | Detect Pose | Frame → 33 Landmarks |
| 3.0 | Calibrate | Landmarks → Calibration Data |
| 4.0 | Analyze Heuristics | Landmarks → Feedbacks |
| 5.0 | Calculate Score | Feedbacks → Summary |
| 6.0 | Render Canvas | Violations → Canvas |
| 7.0 | Play Audio | Errors → TTS |

### ข.5.4 Activity Diagrams

> 📐 **UML Diagrams:** ดู [/docs/diagrams/](../../diagrams/)

| Use Case | Activity Diagram | Description |
|----------|------------------|-------------|
| UC-01 | [ActivityDiagram_UC01.wsd](../../diagrams/ActivityDiagram_UC01.wsd) | T-Pose → 3 sec → บันทึกสัดส่วน |
| UC-02 | [ActivityDiagram_UC02.wsd](../../diagrams/ActivityDiagram_UC02.wsd) | Calibrate → Loop(ตรวจท่า) → Score |
| UC-03 | [ActivityDiagram_UC03.wsd](../../diagrams/ActivityDiagram_UC03.wsd) | Score → Grade → Top Errors |
| UC-04 | [ActivityDiagram_UC04.wsd](../../diagrams/ActivityDiagram_UC04.wsd) | Tutorial → เลือกหัวข้อ |
| UC-05 | [ActivityDiagram_UC05.wsd](../../diagrams/ActivityDiagram_UC05.wsd) | Setting → บันทึก LocalStorage |
| UC-06 | [ActivityDiagram_UC06.wsd](../../diagrams/ActivityDiagram_UC06.wsd) | บันทึก → Download |
| Heuristics | [ActivityDiagram_Heuristics.wsd](../../diagrams/ActivityDiagram_Heuristics.wsd) | ตรวจ 9 กฎ → Feedback |

---

## ข.6 ข้อกำหนดความต้องการ (Requirements Specification)

### ข.6.1 UC-01: Calibrate Body

#### User Requirements (URS)

| ID | Requirement |
|----|-------------|
| URS-01 | ระบบต้องเริ่ม Calibration อัตโนมัติเมื่อกดปุ่ม "เริ่มการฝึก" |
...
| SRS-02-04 | ระบบต้องตรวจสอบตาม 9 กฎของไท่จี๋ |
...
### ข.6.3 UC-03: View Training Result
...
### ข.6.6 UC-06: Manage Reference Data
...
## ข.7 ข้อกำหนดที่ไม่ใช่ฟังก์ชัน (Non-Functional Requirements)
...
### ข.7.6 Internationalization & Accessibility
...
## ข.8 Traceability Matrix

### ข.8.1 Feature → Use Case
...
## ข.9 Glossary (คำศัพท์)

### ข.9.1 คำศัพท์ไท่จี๋

| คำศัพท์ | ภาษาจีน | คำอธิบาย |
|--------|--------|---------| 
| **Silk Reeling** | 纏絲勁 | ท่าม้วนไหม - การเคลื่อนไหวพื้นฐานของมวยไท้เก๊กตระกูลเฉิน |
| **T-Pose** | - | ท่ายืนกางแขนระดับไหล่ ใช้สำหรับ Calibration |
...
| **Vertical Stability** | 虚领顶劲 | โปรงกระหม่อมเบา - ศีรษะตั้งตรงนิ่ง |
...
| คำศัพท์ | คำอธิบาย |
|--------|---------| 
| **Landmark** | จุดพิกัดบนร่างกาย จาก MediaPipe Pose มี 33 จุด |
...
| **Feedback** | ข้อความแจ้งเตือนเมื่อทำท่าผิด |
| **Score** | คะแนน 0-100% คำนวณจาก (เฟรมถูก / เฟรมทั้งหมด) × 100 |
| **Grade** | เกรด A-F แปลงจากคะแนน |
...
| **TTS** | Web Speech API สำหรับอ่านข้อความเป็นเสียง |
| **LocalStorage** | ที่เก็บข้อมูลในเบราว์เซอร์ |
| **Frame** | หนึ่งภาพจากกล้อง (~30 fps) |
| **Visibility** | ค่าความมั่นใจว่าเห็น Landmark (0-1) |

---

## ข.10 Acceptance Criteria (เกณฑ์การยอมรับ)

### ข.10.1 เกณฑ์ระดับ Feature

| Feature | Acceptance Criteria | ค่าอ้างอิงจาก Code |
|---------|---------------------|-------------------|
| **F-01 Calibration** | Calibrate สำเร็จเมื่อยืน T-Pose นิ่ง 3 วินาที | `REQUIRED_STABLE_FRAMES = 90` |
| **F-02 Training** | แสดง Feedback ภายใน 1 วินาที | `FEEDBACK_HOLD_TIME_MS = 1000` |
| **F-03 9 Rules** | ตรวจกฎ 9 ข้อครบตาม Level | Rules 1-9 ใน `heuristics_engine.js` |
| **F-04 Feedback** | แสดงทั้งภาพและเสียง TTS | `showFeedback()` + `speakThai()` |
| **F-05 Result** | คำนวณคะแนนและตัดเกรดถูกต้อง | A≥85, B≥70, C≥55, D≥40, F<40 |

### ข.10.2 เกณฑ์ระดับกฎ (9 Heuristics Rules)

| Rule | ชื่อกฎ | เกณฑ์การตัดสิน | ค่า Threshold |
|:----:|--------|---------------|--------------|
| R-01 | Path Shape | Shape Consistency ≥ 60% | `SHAPE_CONSISTENCY_THRESHOLD = 0.6` |
| R-02 | Arm Rotation | ตรวจเมื่อแขนขยับ ≥ 1.5% | `ARM_MOTION_THRESHOLD = 0.015` |
| R-03 | Elbow Sinking | ศอกต่ำกว่า/เท่าข้อมือ | `ELBOW_TOLERANCE_DEFAULT = 0.01` |
| R-04 | Waist Initiation | สะโพกหมุน ≥ 2°/วินาที | `MIN_HIP_VELOCITY = 2.0` |
| R-05 | Vertical Stability | ศีรษะขยับไม่เกิน 5% | `STABILITY_THRESHOLD_DEFAULT = 0.05` |
| R-06 | Smoothness | Acceleration ≤ 0.05 | `SMOOTHNESS_THRESHOLD_DEFAULT = 0.05` |
| R-07 | Continuity | หยุดไม่เกิน 15 frames | `PAUSE_FRAME_THRESHOLD = 15` |
| R-08 | Weight Shift | น้ำหนักในฐาน ±10% | `WEIGHT_BUFFER_RATIO = 0.1` |
| R-09 | Coordination | Correlation ≥ 0.4 | `COORDINATION_THRESHOLD = 0.4` |

### ข.10.3 เกณฑ์ระดับระบบ

| หมวด | เกณฑ์ | ค่าอ้างอิง |
|------|------|-----------| 
| **Low Light** | เตือนเมื่อ visibility < 50% | `LOW_LIGHT_THRESHOLD = 0.5` |
| **Camera Error** | แจ้งเตือน 4 ประเภท | `showCameraError()` |
| **Scoring** | = (Correct / Total) × 100 | `ScoringManager.getCurrentScore()` |
| **Grading** | A≥85, B≥70, C≥55, D≥40, F<40 | `ScoringManager.getGrade()` |

---

## ข.11 Interface Requirements (ข้อกำหนด Interface)

### ข.11.1 External Interfaces

| Interface | Type | Version | Required |
|-----------|------|---------|:--------:| 
| **MediaPipe Pose** | JS Library | @latest | ✓ |
| **MediaPipe Tasks Vision** | ES Module | @0.10.8 | ✓ |
| **MediaPipe Camera Utils** | JS Library | @latest | ✓ |
| **MediaPipe Drawing Utils** | JS Library | @latest | ✓ |
| **Web Speech API** | Browser API | - | ✓ |
| **Gemini API** | REST API | gemini-2.0-flash-exp | Optional |

### ข.11.2 User Interfaces

| Component | Description | Input | Output |
|-----------|-------------|-------|--------| 
| **Video Canvas** | แสดง Webcam + Overlay | Webcam Stream | Mirrored Video + Skeleton |
| **Control Panel** | ปุ่มเลือกท่า/ระดับ | User Click | State Change |
| **Feedback Overlay** | ข้อความแจ้งเตือน | Heuristics Result | Text + Color |
| **Score Popup** | ผลคะแนนหลังฝึก | Scoring Data | Grade + Stats |
| **Chatbot Widget** | พูดคุยกับ AI | User Message | AI Response |

### ข.11.3 Hardware Interfaces

| Hardware | Requirement | Minimum Spec |
|----------|-------------|--------------| 
| **Webcam** | กล้องถ่ายภาพ | 720p, 30fps |
| **Display** | หน้าจอ | 1280×720 pixels |
| **Speaker** | ลำโพง/หูฟัง | Stereo |
| **CPU** | ประมวลผล | Modern x64/ARM |
| **GPU** | WebGL | WebGL 2.0 compatible |

### ข.11.4 Data Interfaces

| Data | Format | Storage | Description |
|------|--------|---------|-------------| 
| **Reference Path** | JSON | Server (/data) | พิกัดท่าต้นแบบ |
| **Reference Video** | MP4 | Server (/video) | วิดีโอครูผู้สอน |
| **Calibration Data** | JSON | LocalStorage | สัดส่วนร่างกาย |
| **User Settings** | JSON | LocalStorage | ภาษา, ธีม, เสียง |
| **Landmarks** | Array[33] | Memory | x, y, z, visibility |

---

## ข.12 Assumptions & Dependencies

### ข.12.1 Assumptions (สมมติฐาน)
...
| A-04 | ผู้ใช้สวมเสื้อผ้าที่ตัดกับพื้นหลัง |
| A-05 | เบราว์เซอร์รองรับ MediaPipe และ WebRTC |
| A-06 | มีการเชื่อมต่อ Internet สำหรับโหลด AI Model ครั้งแรก |

### ข.12.2 Dependencies (การพึ่งพา)

#### External Libraries (CDN)

| Library | CDN | Version |
|---------|-----|---------|
| MediaPipe Pose | cdn.jsdelivr.net | @latest |
| MediaPipe Tasks Vision | cdn.jsdelivr.net | **@0.10.8** |
| MediaPipe Camera Utils | cdn.jsdelivr.net | @latest |
| MediaPipe Drawing Utils | cdn.jsdelivr.net | @latest |
| TailwindCSS | cdn.tailwindcss.com | @latest |
| Google Fonts (Sarabun) | fonts.googleapis.com | - |
...
|-----|----------|:--------:| 
| getUserMedia | ไม่มี | ✓ |
| Web Speech API | Audio disabled | Optional |
| LocalStorage | Memory fallback | ✓ |
| Fullscreen API | Normal mode | Optional |
| WebGL 2.0 | Software render | ✓ |

### ข.12.3 Constraints (ข้อจำกัด)
...
| 3.0 | 2026-01-09 | - | เพิ่ม F-09~F-11, Exception Flows, NFR-09~10, Glossary, Acceptance Criteria |
| 3.1 | 2026-01-17 | - | ย้ายเป็น Appendix B ตามมาตรฐาน ISO 29110 |
