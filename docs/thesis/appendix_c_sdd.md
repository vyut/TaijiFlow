# ภาคผนวก ค: เอกสาร Software Design Document (SDD)
...
| รายการ | รายละเอียด |
|--------|-----------|
| **ชื่อเอกสาร** | TaijiFlow AI - Software Design Document |
| **Version** | 1.0 |
| **สถานะ** | Final |
...
## ค.3 บทนำ (Introduction)

### ค.3.1 วัตถุประสงค์ (Purpose)
...
### ค.3.3 เอกสารอ้างอิง (References)
...
| **UML Diagrams** | 17 ไฟล์ใน `/docs/diagrams/` |

### ค.3.4 คำศัพท์เฉพาะ (Definitions)
...
## ค.4 การออกแบบสถาปัตยกรรม (Architectural Design)

### ค.4.1 ภาพรวมสถาปัตยกรรม
...
### ค.4.2 Layer Architecture (4 Layers)
...
## ค.5 การออกแบบโมดูล (Module Design)

### ค.5.1 Module Overview
...
class HeuristicsEngine {
    ...
    - checkCoordination()     // Rule 9: ความสัมพันธ์บนล่าง
}
```
...
| R-09 | Upper-Lower Coordination | มือและเท้าเคลื่อนไหวสัมพันธ์กัน | L3 |
...
## ค.6 การออกแบบข้อมูล (Data Design)

### ค.6.1 Data Flow Diagram
...
## ค.7 การออกแบบส่วนต่อประสาน (Interface Design)

### ค.7.1 Module Interfaces
...
## ค.8 การออกแบบ User Interface (UI Design)
...
| **Activity** | ActivityDiagram_UC01.wsd | UC-01: Calibrate Body |
...
| **Activity** | ActivityDiagram_UC05.wsd | UC-05: Manage Settings |
...
### ค.9.2 Use Case Diagram Overview
...
```text
   ...
   │  Trainee │────────│  │  (UC-02) Perform Training  ◄────────┐       │ │
   │  (ผู้ฝึก) │        │  │  (UC-03) View Training Results       │       │ │
   └──────────┘        │  │              ▲                      │       │ │
                        │  │              │<<include>>          │       │ │
                        │  │              └───UC-02─────────────┘       │ │
                        │  └─────────────────────────────────────────────┘ │
   ...
   │  Trainee │────────│  │  (UC-04) View Tutorial                      │ │
   └──────────┘        │  │  (UC-05) Manage Settings                    │ │
                        │  └─────────────────────────────────────────────┘ │
                        │  ┌─────────────────────────────────────────────┐ │
   ┌──────────┐        │  │         Admin Tools (ผู้ดูแล)               │ │
   │  Admin/  │────────│  │  (UC-06) Manage Reference Data              │ │
   │Instructor│        │  └─────────────────────────────────────────────┘ │
   └──────────┘        └──────────────────────────────────────────────────┘
```

### ค.9.3 State Diagram - Training Session
...
### ค.9.4 Class Diagram Overview

> 📐 **Full Diagram:** [ClassDiagram.wsd](../../diagrams/ClassDiagram.wsd)

**22 Classes ใน 6 Packages:**

| Package | Classes | Description |
|---------|:-------:|-------------|
| **Main Controller** | 1 | script.js (Entry Point, Glue Code) |
...
| **UI Managers** | 8 | UIManager, AudioManager, TutorialManager, ScorePopupManager, GestureManager, FeedbackManager, RulesConfigManager, Chatbot |
| **Controllers** | 2 | KeyboardController, DisplayController |
| **Utilities** | 5 | SessionManager, DataExporter, PathGenerator, Translations, SilkAnimation |

**Key Relationships:**

```
script.js ─────┬───> HeuristicsEngine ──> ScoringManager
               ├───> CalibrationManager
               ├───> DrawingManager
               ├───> UIManager
               ├───> AudioManager
...
### ค.9.5 Sequence Diagrams

> 📐 **Full Diagrams:** [/docs/diagrams/](../../diagrams/)

#### ค.9.5.1 Training Flow (Main Sequence)

![Training Flow](../../out/docs/diagrams/SequenceDiagram_TrainingFlow/SequenceDiagram_TrainingFlow.svg)

| Phase | Participants | Description |
|:-----:|--------------|-------------|
| 1 | User → UI → Training | เลือกท่า → กด Start/👍 → เริ่ม Calibration |
| 2 | Training → Calibrator | T-Pose → 3 sec → บันทึกสัดส่วน |
| 3 | Training → Heuristics → Scorer | Loop: ตรวจท่า → Feedback → บันทึกคะแนน |
| 4 | Training → UI | Stop/✊/timeout → แสดง Score Popup |

#### ค.9.5.2 Calibration Sequence

![Calibration](../../out/docs/diagrams/SequenceDiagram_Calibration/SequenceDiagram_Calibration.svg)

| Step | Action |
|:----:|--------|
| 1 | User ยืน T-Pose |
| 2 | CalibrationManager ตรวจ Visibility |
| 3 | ถ้า Visibility < threshold → Error |
| 4 | นับ 3 วินาที (stable frames) |
| 5 | คำนวณ armLength, shoulderWidth, torsoHeight |
| 6 | บันทึก LocalStorage → ส่ง HeuristicsEngine |

#### ค.9.5.3 Realtime Analysis Sequence

![Realtime Analysis](../../out/docs/diagrams/SequenceDiagram_RealtimeAnalysis/SequenceDiagram_RealtimeAnalysis.svg)

| Loop | FPS | Description |
|------|:---:|-------------|
| Camera Loop | 30 | รับ Frame → วาด Skeleton |
| AI Loop | ~7.5 | Throttle → 8 Rules Check → Feedback |

---

## ค.10 Design Patterns

### ค.10.1 Patterns Used
...
## ค.13 Traceability Matrix (SRS → SDD)

### ค.13.1 Functional Requirements to Modules
...
### ค.13.2 Non-Functional Requirements to Design
...
### ค.14.2 Version History

| Version | Date | Author | Changes |
|:-------:|------|--------|---------|
| 1.0 | 2026-01-09 | TaijiFlow Team | Initial SDD based on ISO 29110 |
| 1.1 | 2026-01-17 | TaijiFlow Team | ย้ายเป็น Appendix C ตามมาตรฐาน ISO 29110 |
