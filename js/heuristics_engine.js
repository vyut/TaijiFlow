 ...
 * ============================================================================
 *
 * ระบบวิเคราะห์ท่าทางการฝึกม้วนไหม (Silk Reeling) ตามหลักไทเก๊กเฉินสไตล์
 *
 * 📋 หน้าที่หลัก:
 *   - วิเคราะห์ท่าทางจาก MediaPipe landmarks แบบ real-time
 *   - เปรียบเทียบกับ Reference Path และหลักการที่ถูกต้อง
 *   - ส่ง Feedback ข้อผิดพลาดไปแสดงบนหน้าจอ
 *
 * ⚙️ Features (v3.0):
 ...
 *   2. Waist Initiation   - เอวเป็นจักรเพลา (腰为轴)
 *   3. Weight Shift       - ถ่ายน้ำหนักอยู่ในฐาน
 *   4. Vertical Stability - ศีรษะนิ่ง (โปรงกระหม่อมเบา)
 *   5. Arm Rotation       - หมุนฝ่ามือถูกทิศ (หงาย/คว่ำ)
 *   6. Elbow Sinking      - ศอกจม ไม่ลอย (沉肩坠肘)
...
class HeuristicsEngine {
  constructor() {
    ...
    // =========================================================================
    // L1 (ท่านั่ง): ไม่เช็ค Stability และ Weight Shift (เพราะนั่งอยู่)
    // L2 (ท่ายืน): เพิ่ม Stability, ยังไม่เช็ค Weight Shift
    // L3 (ท่ายืนย่อ): เช็คทุกอย่าง (ยากที่สุด)
    // =========================================================================
    this.RULES_CONFIG = {
      L1: {
        // --- Level 1: ท่านั่ง (สำหรับผู้เริ่มต้น) ---
        // เน้นการหมุนแขน ไม่ต้องห่วงสมดุล (นั่งอยู่)
        checkPath: true, // เส้นทางแม่นยำ
        checkRotation: true, // การหมุนแขน
        checkElbow: true, // ศอกจม
        checkWaist: true, // เอวนำ
        checkStability: false, // ✘ ไม่เช็ค (นั่ง = ศีรษะนิ่งอยู่แล้ว)
        checkSmooth: true, // ความลื่นไหล
        checkContinuity: true, // ความต่อเนื่อง
        checkWeight: false, // ✘ ไม่เช็ค (นั่ง = ไม่ต้องถ่ายน้ำหนัก)
      },
      L2: {
        // --- Level 2: ท่ายืน (ระดับกลาง) ---
        // เพิ่ม Stability (ยืนแล้วศีรษะอาจขยับ)
        checkPath: true,
        checkRotation: true,
        checkElbow: true,
        checkWaist: true,
        checkStability: true, // ✔ เพิ่ม: ตรวจศีรษะนิ่ง
        checkSmooth: true,
        checkContinuity: true,
        checkWeight: false, // ✘ ยังไม่เช็ค (ยืนตรง = ไม่ถ่ายน้ำหนัก)
      },
      L3: {
        // --- Level 3: ท่ายืนย่อ/ขาคู่ (ยากที่สุด) ---
        // เช็คทุกกฎ รวมถึงการถ่ายน้ำหนัก (Bow Stance)
        checkPath: true,
        checkRotation: true,
        checkElbow: true,
        checkWaist: true,
        ...
        checkSmooth: true,
        checkContinuity: true,
        checkWeight: true, // ✔ เพิ่ม: ตรวจการถ่ายน้ำหนัก
      },
    };
    ...
    // =========================================================================
    this.RULE_PRIORITY = {
      // Priority 1-3: หลักพื้นฐาน (ผิดแล้วไม่ใช่ไทเก๊ก)
      "Path Accuracy": 1, // 🥇 ท่าผิดเลย - สำคัญที่สุด
